...
import { Session, SessionConfig, SessionStatus } from "../models/models.js"

export function Create(config: SessionConfig): Promise<Session | null> {
    return ipc("SessionService.Create", config).then(createNullable(Session.createFrom))
}
...
export function GetAll(): Promise<Session[]> {
    return ipc("SessionService.GetAll").then(createArray(Session.createFrom))
}

export function GetByID(id: string): Promise<Session | null> {
    return ipc("SessionService.GetByID", id).then(createNullable(Session.createFrom))
}
...
export function UpdateStatus(id: string, status: SessionStatus): Promise<void> {
