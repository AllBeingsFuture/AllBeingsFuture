...
describe('chatCore.remove recursive descendants', () => {
  ...
  it('G→F→S remove(G): only unrelated remains; agents/childToParent cleaned; StopProcess on all three', async () => {
    const gId = 'grandpa-1'
    const fId = 'father-1'
    const sId = 'son-1'
    ...
    const snapshot = emptySnapshot({
      ...
      chatError: 'x',
      sessions: [
        makeSession({ id: gId, name: 'G' }),
        makeSession({ id: fId, name: 'F', parentSessionId: gId }),
        makeSession({ id: sId, name: 'S', parentSessionId: fId }),
        makeSession({ id: otherId, name: 'Other' }),
      ],
      agents: {
        [gId]: [agentF],
        [fId]: [agentS],
    ...
    expect(serviceMocks.sessionService.Delete).toHaveBeenCalledTimes(1)
    expect(serviceMocks.sessionService.Delete).toHaveBeenCalledWith(gId)
    expect(serviceMocks.processService.StopProcess).toHaveBeenCalledWith(gId)
    expect(serviceMocks.processService.StopProcess).toHaveBeenCalledWith(fId)
    expect(serviceMocks.processService.StopProcess).toHaveBeenCalledWith(sId)
    expect(serviceMocks.processService.StopProcess).toHaveBeenCalledTimes(3)
