import { Loader2, SendHorizonal, Square, X, Upload, Paperclip, Smile, FileIcon } from 'lucide-react'
...
export default function MessageInput({
  ...
  const addFileByPath = useCallback(async (filePath: string) => {
    try {
      ...
      const size = prepared.sizeBytes < 1024
  ...
  const submit = useCallback(async () => {
    ...
    if ((!text && images.length === 0 && files.length === 0) || disabled) return
    ...
    await onSend(finalText, attachedImages)
  }, [clearDraft, disabled, files, images, onSend, sessionId, value])
            ...
            {files.map((file, index) => (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="max-w-[200px] truncate text-[10px] text-gray-500" title={file.path}>{file.path}</p>
