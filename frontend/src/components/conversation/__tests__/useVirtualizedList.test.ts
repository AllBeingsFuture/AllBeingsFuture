...
describe('useVirtualizedList', () => {
  ...
  it('keeps sticky measured size when estimate grows without remeasure', () => {
    // Default estimate/measure: measured wins over a larger estimate (no growing-estimate path).
    type Row = { id: string; size: number; content: string }
    const { result, rerender } = renderHook(({ items }) => useVirtualizedList({
      items,
      enabled: true,
      getItemKey: (item) => item.id,
    ...
    // Estimate grows; layout height stays on last measured until RO updates.
    rerender({
      items: [
        { id: 'a', size: 160, content: 'hi there longer' },
        { id: 'b', size: 50, content: 'done' },
      ],
    })
    expect(result.current.totalHeight).toBe(130)

    // Large overestimate still does not inflate totalHeight past measured.
    rerender({
      items: [
        { id: 'a', size: 300, content: 'finalized long body' },
        { id: 'b', size: 50, content: 'done' },
      ],
    })
    expect(result.current.totalHeight).toBe(130)

    // RO reports real growth → totalHeight follows measured.
    act(() => {
      node.__height = 200
