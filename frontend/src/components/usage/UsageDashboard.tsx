 ...
 * and a session breakdown pie chart. Fetches real data via usageStore;
...
export default function UsageDashboard() {
  const { summary, dailyStats, sessionStats, loading, fetchUsage } = useUsageStore()

  const refresh = useCallback(() => { fetchUsage(30) }, [fetchUsage])
  ...
  const barData = useMemo(
    () => dailyStats.map((d) => ({ ...d, date: formatDate(d.date) })),
    [dailyStats],
  )
  ...
  const pieData = useMemo(() => {
    ...
    const sorted = [...sessionStats].sort((a, b) => b.tokens - a.tokens)
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="rounded-lg border border-border bg-bg-primary p-2.5">
          <div className="mb-1 flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-text-muted">活跃会话</span>
          </div>
      ...
      {/* Empty State */}
      {barData.length === 0 && pieData.length === 0 && !loading && (
