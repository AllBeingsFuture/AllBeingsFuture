 ...
 *
 * This version imports soffice helpers from the office module.
 */
...
// Inline soffice env helper (the office module is handled separately)
function getSofficeEnv(): NodeJS.ProcessEnv {
  const env = { ...process.env };
  env.SAL_USE_VCLPLUGIN = "svp";
  // On Linux, check if AF_UNIX sockets are blocked and apply LD_PRELOAD shim if needed
  // This is a simplified version; the full shim logic is in the office module
  return env;
}

const MACRO_DIR_MACOS =
  "~/Library/Application Support/LibreOffice/4/user/basic/Standard";
const MACRO_DIR_LINUX =
...
function hasGtimeout(): boolean {
  try {
    const r = spawnSync("gtimeout", ["--version"], {
      stdio: "pipe",
      timeout: 1000,
    });
    return r.status === 0;
...
async function recalc(
  ...
  if (result.status !== 0 && result.status !== 124) {
    const errorMsg =
      result.stderr || "Unknown error during recalculation";
    if (
  ...
  try {
    ...
    output.total_formulas = formulaCount;
    return output;
  } catch (e: unknown) {
...
async function main(): Promise<void> {
  ...
  const resultData = await recalc(filename, timeout);
