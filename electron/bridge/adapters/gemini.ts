/**
 * Gemini CLI Headless Adapter (TypeScript port)
 *
 * Spawns `gemini` CLI in single-turn mode with NDJSON output.
 * Each send() creates a new process. Session context maintained via --session-id.
 */
...
export class GeminiAdapter {
  ...
  async send(message: string, _images?: any[]): Promise<void> {
    ...
    const { cmd, args: baseArgs } = parseCommand(this.config.command, 'gemini')
    ...
    return new Promise<void>((resolve) => {
      ...
      rl.on('line', (line: string) => {
        ...
        try {
          ...
          if (data.text) {
            ...
            this.emit({ id: requestId, event: 'delta', text: data.text })
          }
          if (data.session_id) {
      ...
      this.currentProcess.on('exit', (code) => {
        ...
        if (!currentText && stderrBuf.trim()) {
          ...
          this.emit({ id: requestId, event: 'error', error: `Gemini process exited (code ${code})` })
        } else {
          this.emit({ id: requestId, event: 'done', text: currentText, conversationId: this.sessionId || '' })
        }
        resolve()
      })
    })
