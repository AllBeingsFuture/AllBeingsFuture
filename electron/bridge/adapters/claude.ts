...
export class ClaudeAdapter {
  ...
  private startQuery(message: string, images?: Array<{data: string, mimeType: string}>) {
    ...
    if (autoAccept) {
      ...
      // Headless SDK integration — no interactive terminal to answer prompts.
      // Default to bypass; session config should set autoAccept=false + permissionMode
      // explicitly if stricter control is desired.
      options.permissionMode = 'bypassPermissions'
      options.allowDangerouslySkipPermissions = true
    }
        ...
        switch (msg.type) {
          ...
          case 'stream_event': {
            ...
            if (event?.delta?.type === 'text_delta' && event.delta.text) {
              ...
              this.emit({ id: requestId, event: 'thinking', text: event.delta.thinking })
              ...
              // Tool input streaming — ignore (we already emitted tool event on content_block_start)
            }
            break
          }
          ...
          case 'result': {
            ...
            this.emit({
              ...
              usage: msg.usage || msg.data?.usage || undefined,
            })
            currentText = ''
            this._resultEmitted = true
            break
  ...
  /**
   * Check whether this adapter has an active query/stream.
   * Used by BridgeManager to determine if the session is truly alive.
   */
  isAlive(): boolean {
    // Session is alive if it has an active query OR is in the process of consuming a stream
    return this.sdkQuery !== null || this.consuming
  }
