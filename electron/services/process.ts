...
import { injectSupervisorPrompt, cleanupSupervisorPrompt } from './supervisor-prompt.js'
...
export class ProcessService {
   ...
   * (Pattern borrowed from SpectrAI's AgentManagerV2.agentIdleFlags)
   */
  ...
  async initSession(sessionId: string): Promise<void> {
    ...
    // Inject agent-control MCP server for non-child Claude sessions
    if (!session.parentSessionId && this.isClaudeAdapter(provider.adapterType)) {
      try {
      ...
      // Inject supervisor prompt rules file so Claude knows how to use agent-control tools
      try {
        ...
        appLog('warn', `Failed to inject supervisor prompt: ${err.message}`, 'process')
      }
    }
    ...
    try {
      ...
      this.concurrencyGuard.registerSession()
  ...
  private handleBridgeEvent(sessionId: string, event: any) {
    ...
    switch (event.event) {
      ...
      case 'tool': {
        // Append tool use info to last assistant message
        const last = state.messages[state.messages.length - 1]
        if (last?.role === 'assistant') {
          if (!last.toolUse) last.toolUse = []
          last.toolUse.push({ name: event.name, input: event.input })
        }
        this.emitChatUpdate(sessionId)
        break
      }
  ...
  async stopProcess(sessionId: string): Promise<void> {
    ...
    this.concurrencyGuard.unregisterSession()
    this.sessionService.updateStatus(sessionId, 'idle')
    this.emitChatUpdate(sessionId)
  ...
  async closeChildSession(parentSessionId: string, childSessionId: string): Promise<void> {
    ...
    // Stop the child's bridge adapter
  ...
  private finalizeChildAgents(
    ...
    for (const agent of finalized) {
      // Destroy live adapter for persistent children
  ...
  private handleAgentTaskEvent(parentSessionId: string, event: any) {
    ...
    if (agent) {
      // Force-reload child session messages from DB into in-memory state,
      // because AgentTracker writes directly to DB and the in-memory state
  ...
  // ─── Agent idle detection (borrowed from SpectrAI AgentManagerV2) ───
